...
class StanPlanu:
    ...
    def czy_ruch_jest_legalny(self, zajecia, proponowany_dzien, start_slot, sala, prowadzacy):
        ...
        #Limit maksymalnie 10 godzin tygodniowo dla prowadzącego
        ...
        for offset in range(zajecia.wymagane_godziny):
            ...
            #Blokada twarda na podstawie LLM
...
class AlgorytmKonstruktywny:
    def __init__(self, stan_planu, prowadzacy_db, sale_db, przedmioty_db):
        ...
        #Najdłuższe i największe zajęcia wstawiane jako pierwsze
    ...
    def _backtrack(self, indeks_zajec):
        ...
        dostepni_prowadzacy = [p for p in self.prowadzacy_db.values() if zajecia.baza_przedmiotu in p.kompetencje]  #filtr kompetencji
        ...
        for prowadzacy in dostepni_prowadzacy:
            for sala in self.sale_db:
                if sala.typ != zajecia.wymagany_typ_sali or sala.pojemnosc < zajecia.liczba_studentow:  #filtr sal
                    continue 
...
class AlgorytmWyzarzania:
    ...
    def __init__(self, stan_planu, lista_zajec, prowadzacy_db, sale_db):
        ...
        self.WAGA_OKIENKO = 50 #kalkulator kar
        self.WAGA_NIECHETNIE = 80 

    def oblicz_koszt(self): #ocenia jakość planu
        koszt = 0
        harmonogram_prow = {} 
        ...
        for zajecia in self.lista_zajec:
            ...
            #1: Ocena na podstawie macierzy od LLM (czy trafiliśmy w preferowaną godzinę)
            if hasattr(prowadzacy, 'availability_matrix') and prowadzacy.availability_matrix:
                matryca = prowadzacy.availability_matrix
        ...
        #2: Liczenie okienek dla kadry naukowej
    ...
    def optymalizuj(self, temp_pocz=1000.0, temp_konc=1.0, alfa=0.98, iter_na_temp=200):
        ...
        while temp > temp_konc:
            for _ in range(iter_na_temp):
                #wybieramy losowe zajęcia do przeniesienia
                zajecia = random.choice(self.lista_zajec)
                
                #zabezpieczamy obecny stan
                ...
                if not znaleziono_miejsce:
                    self.stan.wstaw_zajecia(zajecia, stary_dzien, stary_slot, stara_sala, prowadzacy) #odrzucenie ruchu
                    continue
                    
                #EWALUACJA NOWEGO STANU
